...
const Section=({title,children})=><section className="mt-10"><h2 className="font-heading text-3xl md:text-4xl font-semibold text-[#17364B]">{title}</h2><div className="mt-4 space-y-4 text-[#526B7A] leading-relaxed">{children}</div></section>;

export default function Privacy(){return <><Seo title="Privacy Policy" description="Privacy Policy for Consecrated Hands."/><main className="pt-40 pb-24 bg-white"><div className="max-w-4xl mx-auto px-6 md:px-12"><p className="text-xs uppercase tracking-[.28em] text-[#9B7412] font-bold">Privacy</p><h1 className="mt-5 font-heading text-5xl md:text-7xl font-semibold text-[#17364B]">Privacy Policy</h1><p className="mt-5 text-sm text-[#526B7A]">Effective August 29, 2026</p><p className="mt-8 text-lg text-[#405E70] leading-relaxed">Consecrated Hands respects the privacy of young people, families, volunteers, donors, prayer-request submitters, and community partners. This policy explains the information our website may collect, why we collect it, and the choices available to you.</p>
...
<Section title="Contact us"><p>Questions about privacy can be sent to <a className="font-bold text-[#176F9F] hover:underline" href={`mailto:${ORG.email}`}>{ORG.email}</a>.</p></Section>
</div></main></>}
