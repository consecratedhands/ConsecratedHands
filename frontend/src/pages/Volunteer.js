...
export default function Volunteer(){return <>
  ...
  <section className="pt-24 md:pt-28 bg-white"><div className="max-w-7xl mx-auto px-6 md:px-12 py-12 sm:py-16 md:py-24 grid lg:grid-cols-[1.05fr_.95fr] gap-12 items-center">
    <Reveal><div><p className="text-xs uppercase tracking-[.28em] text-[#A66F00] font-bold">Get Involved</p><h1 className="mt-5 font-heading text-[2.65rem] sm:text-5xl md:text-7xl lg:text-[5.2rem] font-semibold text-[#25292C]">Be the person who <span className="italic text-[#007BC2]">shows up.</span></h1><p className="mt-7 max-w-3xl text-xl text-[#424B52] leading-relaxed">Young people remember consistency. Whether you mentor, tutor, share a career, serve at an event, partner as a church or business, or help behind the scenes, your presence can strengthen the web around a young life.</p><Link to="/contact" className="inline-flex items-center gap-2 mt-8 bg-[#6FD3FF] text-[#25292C] px-7 py-3.5 rounded-full font-extrabold">Start the Conversation <ArrowRight size={17}/></Link></div></Reveal>
    <Reveal delay={.08}><img src={IMAGES.volunteering} alt="Volunteers serving together" className="w-full aspect-[4/4.6] object-cover rounded-[2.25rem] shadow-[0_28px_70px_rgba(35,105,145,.14)]"/></Reveal>
  </div></section>
  ...
  <section className="py-16 sm:py-20 md:py-32 bg-white"><div className="max-w-7xl mx-auto px-6 md:px-12 grid lg:grid-cols-[.9fr_1.1fr] gap-12 items-center">
    ...
    <Reveal delay={.06}><div className="rounded-[2rem] bg-[#F4FBFF] border border-[#D8EEF8] p-7 md:p-9"><div className="space-y-4">{standards.map((s,i)=><div key={s} className="flex gap-4"><div className="w-9 h-9 rounded-full bg-[#6FD3FF] text-[#25292C] flex items-center justify-center font-extrabold shrink-0">{i+1}</div><p className="text-[#424B52] leading-relaxed pt-1">{s}</p></div>)}</div></div></Reveal>
  </div></section>
