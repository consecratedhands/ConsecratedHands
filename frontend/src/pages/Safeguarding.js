...
export default function Safeguarding(){return <><Seo title="Youth Safeguarding" description="Consecrated Hands youth safeguarding commitment."/><main className="pt-40 pb-24 bg-white"><div className="max-w-4xl mx-auto px-6 md:px-12"><p className="text-xs uppercase tracking-[.28em] text-[#9A6B00] font-bold">Youth Safety</p><h1 className="mt-5 font-heading text-[2.65rem] sm:text-5xl md:text-7xl font-semibold text-[#25292C]">Youth Safeguarding Commitment</h1><p className="mt-8 text-lg text-[#47535B] leading-relaxed">Young people deserve ministry and mentorship environments that protect their dignity, safety, privacy, and God-given worth. Consecrated Hands is committed to building youth programs around appropriate boundaries, responsible adult conduct, transparency, and prompt action when safety concerns arise.</p>
...
<Section title="Contact"><p>Questions about youth safety or this commitment may be sent to <a className="font-bold text-[#1683BD] hover:underline" href={`mailto:${ORG.email}`}>{ORG.email}</a>. Emergency or legally reportable concerns should be directed to the appropriate authorities rather than waiting for an email response.</p></Section>
</div></main></>}
