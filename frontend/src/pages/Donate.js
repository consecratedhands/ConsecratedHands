...
import { Heart, HandHeart, Loader2, Lock, ShieldCheck, WalletCards } from "lucide-react";
...
const givingDetails=[
  {icon:Lock,title:"Secure checkout",body:"Online gifts are processed through Stripe so donors can use a trusted payment system."},
  {icon:WalletCards,title:"One-time or monthly",body:"Give once or choose recurring support when that fits your budget and intention."},
  {icon:HandHeart,title:"Responsible stewardship",body:"Gifts support the real costs of youth mentorship, program delivery, outreach, and responsible nonprofit operations."},
];
...
export default function Donate(){
 ...
 const donate=async()=>{
   ...
   if(!Number.isFinite(chosenAmount)||chosenAmount<1||chosenAmount>100000){toast.error("Please choose a donation amount between $1 and $100,000.");return;}
   if(!API){toast.error(`Online giving is being connected. Please contact us at ${ORG.email}.`);return;}
   setLoading(true);
   try{
     const {data}=await axios.post(`${API}/donations/checkout`,{amount:chosenAmount,frequency,donor_name:donor.name.trim(),donor_email:donor.email.trim(),website:donor.website});
     const checkoutUrl=new URL(data.checkout_url);
     if(checkoutUrl.protocol!=="https:"||!checkoutUrl.hostname.endsWith(".stripe.com"))throw new Error("Unexpected checkout URL");
 ...
 return <>
   ...
   <section className="relative pt-24 md:pt-28 overflow-hidden light-glow"><div className="relative max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-28"><Reveal><div className="max-w-3xl"><p className="text-xs uppercase tracking-[.28em] text-[#A66F00] font-bold">Give With Purpose</p><h1 className="mt-5 font-heading text-[2.65rem] sm:text-5xl md:text-7xl lg:text-[5.2rem] font-semibold text-[#25292C]">Help make consistent mentorship <span className="italic text-[#007BC2]">possible.</span></h1><p className="mt-7 text-xl text-[#424B52] leading-relaxed">Your gift supports the people, resources, experiences, and operating needs that allow Consecrated Hands to show up for young people with consistency and care.</p></div></Reveal></div></section>

   <section className="py-16 sm:py-20 md:py-24 bg-[#F4FBFF]"><div className="max-w-7xl mx-auto px-6 md:px-12"><Reveal><div className="max-w-4xl"><p className="text-xs uppercase tracking-[.28em] text-[#A66F00] font-bold">Giving Details</p><h2 className="mt-4 font-heading text-[2.6rem] sm:text-5xl md:text-6xl font-semibold text-[#25292C]">Simple, secure, and transparent.</h2></div></Reveal><div className="mt-10 grid md:grid-cols-3 gap-5">{givingDetails.map(({icon:Icon,title,body},i)=><Reveal key={title} delay={i*.04}><div className="h-full bg-white rounded-3xl border border-[#D8EEF8] p-7"><div className="w-12 h-12 rounded-2xl bg-[#EAF9FF] flex items-center justify-center"><Icon className="text-[#006DAA]"/></div><h3 className="mt-5 font-heading text-3xl font-semibold text-[#25292C]">{title}</h3><p className="mt-3 text-[#59636A] leading-relaxed">{body}</p></div></Reveal>)}</div></div></section>
   ...
   <section className="py-16 sm:py-20 md:py-24 bg-white"><div className="max-w-7xl mx-auto px-6 md:px-12 grid lg:grid-cols-[.7fr_1.3fr] gap-10 items-start">
     ...
     <Reveal delay={.05}><form onSubmit={event=>{event.preventDefault();donate()}} className="bg-white rounded-[2rem] border border-[#D8EEF8] shadow-[0_20px_60px_rgba(35,105,145,.08)] p-7 md:p-10"><div className="text-center mb-8"><Heart className="mx-auto text-[#A66F00]"/><p className="mt-4 text-xs uppercase tracking-[.28em] text-[#A66F00] font-bold">Secure Giving</p><h2 className="mt-3 font-heading text-4xl md:text-5xl font-semibold text-[#25292C]">Choose your gift.</h2></div>
       {fallbackOnly?<div className="text-center py-4"><Lock className="mx-auto text-[#A66F00]"/><h3 className="mt-4 font-heading text-3xl font-semibold text-[#25292C]">Secure giving through Stripe</h3><p className="mt-3 text-[#59636A]">Continue to our secure Stripe donation page to choose your amount.</p></div>:<>
         ...
         <div className="mt-5"><label htmlFor="donation-custom" className="sr-only">Custom donation amount</label><input id="donation-custom" type="number" min="1" max="100000" step="0.01" className={input} value={custom} onChange={e=>setCustom(e.target.value)} placeholder="Custom amount" inputMode="decimal"/></div>
         <div className="grid md:grid-cols-2 gap-5 mt-7"><label htmlFor="donor-name" className="text-sm font-semibold text-[#424B52]">Full name<input id="donor-name" autoComplete="name" required className={`${input} mt-2`} value={donor.name} onChange={e=>setDonor(d=>({...d,name:e.target.value}))} placeholder="Your name" maxLength={120}/></label><label htmlFor="donor-email" className="text-sm font-semibold text-[#424B52]">Email<input id="donor-email" autoComplete="email" required type="email" className={`${input} mt-2`} value={donor.email} onChange={e=>setDonor(d=>({...d,email:e.target.value}))} placeholder="you@email.com"/></label></div>
       ...
       <button type="submit" disabled={loading} className="mt-7 w-full rounded-full bg-[#FFD21F] text-[#25292C] py-4 font-extrabold flex items-center justify-center gap-2 shadow-[0_14px_30px_-16px_rgba(210,156,0,.8)] hover:bg-[#FFE066] transition disabled:opacity-60">{loading?<><Loader2 size={18} className="animate-spin"/>Redirecting…</>:<><Heart size={18}/>{fallbackOnly?"Continue to Secure Giving":`Give ${Number.isFinite(chosenAmount)?`$${chosenAmount}`:""} ${frequency==="monthly"?"monthly":"now"}`}</>}</button>
