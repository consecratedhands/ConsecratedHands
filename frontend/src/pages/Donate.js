...
export default function Donate(){
 ...
 return <>
   ...
   <section className="pt-32 md:pt-36 bg-white"><div className="max-w-7xl mx-auto px-6 md:px-12 py-16 md:py-24 grid lg:grid-cols-[1.05fr_.95fr] gap-12 items-center">
     <Reveal><div><p className="text-xs uppercase tracking-[.28em] text-[#8A650C] font-bold">Give With Purpose</p><h1 className="mt-5 font-heading text-5xl md:text-7xl lg:text-[5.2rem] font-semibold text-[#17364B]">Help build the next <span className="italic text-[#2F8FC7]">strand.</span></h1><p className="mt-7 max-w-3xl text-xl text-[#405E70] leading-relaxed">Your gift helps create Christ-centered mentorship, educational support, life-skills development, career exposure, community experiences, and practical opportunities for young people.</p><p className="mt-5 text-[#526B7A] leading-relaxed">We do not use made-up “$25 equals exactly X outcome” claims. Needs vary. Gifts support the real work of building and sustaining the web around young people.</p></div></Reveal>
   ...
   <section className="py-24 bg-[#F8FCFF]"><div className="max-w-7xl mx-auto px-6 md:px-12"><Reveal><div className="max-w-4xl"><p className="text-xs uppercase tracking-[.28em] text-[#8A650C] font-bold">What Your Giving Supports</p><h2 className="mt-4 font-heading text-5xl md:text-6xl font-semibold text-[#17364B]">Resources follow relationships.</h2><p className="mt-6 text-lg text-[#526B7A] leading-relaxed max-w-3xl">Donations help Consecrated Hands create the conditions where mentorship can become practical—time, transportation, learning, experiences, opportunity, communication, and responsible program support.</p></div></Reveal><div className="mt-10 grid md:grid-cols-2 lg:grid-cols-4 gap-5">{supports.map(({icon:Icon,title,body},i)=><Reveal key={title} delay={i*.04}><div className="h-full bg-white rounded-3xl border border-[#DCE8EF] p-7"><div className="w-12 h-12 rounded-2xl bg-[#EAF7FF] flex items-center justify-center"><Icon className="text-[#176F9F]"/></div><h3 className="mt-5 font-heading text-3xl font-semibold text-[#17364B]">{title}</h3><p className="mt-3 text-[#526B7A] leading-relaxed">{body}</p></div></Reveal>)}</div></div></section>
   ...
   <section className="py-24 bg-white"><div className="max-w-7xl mx-auto px-6 md:px-12 grid lg:grid-cols-[.7fr_1.3fr] gap-10 items-start">
     <Reveal><aside className="rounded-[2rem] bg-[#16384F] text-white p-7 md:p-9 lg:sticky lg:top-28"><ShieldCheck className="text-[#F5D875]" size={36}/><h2 className="mt-5 font-heading text-4xl font-semibold">Give with confidence.</h2><p className="mt-4 text-white/75 leading-relaxed">Consecrated Hands is a federally recognized 501(c)(3) public charity. Contributions may be tax-deductible to the fullest extent allowed by law.</p><div className="mt-6 pt-6 border-t border-white/15 space-y-3 text-sm text-white/75"><p><strong className="text-white">EIN:</strong> {ORG.ein}</p><p><strong className="text-white">Payments:</strong> Secure checkout is processed by Stripe.</p><p><strong className="text-white">Stewardship:</strong> We believe faith and professional accountability belong together.</p></div><div className="mt-6 flex flex-col gap-3"><Link to="/about" className="text-[#9AD8FF] font-bold hover:underline">About Consecrated Hands</Link><Link to="/privacy" className="text-[#9AD8FF] font-bold hover:underline">Privacy Policy</Link><Link to="/terms" className="text-[#9AD8FF] font-bold hover:underline">Terms of Use</Link></div></aside></Reveal>

     <Reveal delay={.05}><div className="bg-white rounded-[2rem] border border-[#DCE8EF] shadow-[0_20px_60px_rgba(41,91,123,.08)] p-7 md:p-10"><div className="text-center mb-8"><Heart className="mx-auto text-[#8A650C]"/><p className="mt-4 text-xs uppercase tracking-[.28em] text-[#8A650C] font-bold">Secure Giving</p><h2 className="mt-3 font-heading text-4xl md:text-5xl font-semibold text-[#17364B]">Choose your gift.</h2></div>
       {fallbackOnly?<>
         <div className="text-center py-4"><Lock className="mx-auto text-[#8A650C]"/><h3 className="mt-4 font-heading text-3xl font-semibold text-[#17364B]">Secure giving through Stripe</h3><p className="mt-3 text-[#526B7A]">Continue to our secure Stripe donation page to choose your amount.</p></div>
       </>:<>
         <fieldset><legend className="sr-only">Donation frequency</legend><div className="flex bg-[#F1F8FC] rounded-full p-1 max-w-xs mx-auto mb-8">{[{k:"one_time",l:"One-time"},{k:"monthly",l:"Monthly"}].map(o=><button type="button" aria-pressed={frequency===o.k} key={o.k} onClick={()=>setFrequency(o.k)} className={`flex-1 py-2.5 rounded-full text-sm font-bold transition ${frequency===o.k?"bg-[#73C8FF] text-[#17364B] shadow-sm":"text-[#526B7A]"}`}>{o.l}</button>)}</div></fieldset>
         <fieldset><legend className="text-sm font-semibold text-[#405E70] mb-3">Choose an amount</legend><div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{SUGGESTED.map(a=><button type="button" aria-pressed={!custom&&amount===a} key={a} onClick={()=>{setAmount(a);setCustom("")}} className={`py-3.5 rounded-2xl border font-bold transition ${!custom&&amount===a?"bg-[#EAF7FF] border-[#2F8FC7] text-[#176F9F]":"border-[#D5E4EC] text-[#17364B] hover:border-[#2F8FC7]"}`}>${a}</button>)}</div></fieldset>
   ...
   <section className="py-24 bg-[#F8FCFF]"><div className="max-w-6xl mx-auto px-6 md:px-12 grid lg:grid-cols-[1fr_1fr] gap-12 items-center"><Reveal><img src={IMAGES.youthPortrait} alt="Young person looking toward the future" loading="lazy" className="w-full aspect-[4/3] object-cover rounded-[2rem] shadow-[0_24px_60px_rgba(32,89,124,.12)]"/></Reveal><Reveal delay={.06}><div><p className="text-xs uppercase tracking-[.28em] text-[#8A650C] font-bold">Why Monthly Giving Matters</p><h2 className="mt-4 font-heading text-5xl md:text-6xl font-semibold text-[#17364B]">Consistency helps fund consistency.</h2><p className="mt-6 text-lg text-[#526B7A] leading-relaxed">Mentorship works through relationships over time. Recurring support can help an organization plan more responsibly around ongoing program needs instead of depending only on one-time gifts.</p><p className="mt-5 text-[#526B7A] leading-relaxed">Choose monthly giving only if it fits your budget and intention. One-time gifts are equally appreciated and meaningful.</p></div></Reveal></div></section>
 </>
}
