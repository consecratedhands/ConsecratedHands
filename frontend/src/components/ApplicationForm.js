...
const inputCls="w-full bg-transparent border-b border-[#C9D9E2] py-4 text-[#25292C] placeholder:text-[#708795] focus:outline-none focus:border-[#1683BD] transition-colors duration-300";
const labelCls="block text-xs uppercase tracking-widest text-[#47535B] font-bold mb-2";
...
export default function ApplicationForm({interests=["volunteer","mentor","mentee","partner","general"],defaultInterest="volunteer",submitLabel="Send message",messagePlaceholder="Tell us how you'd like to help, or ask us anything…",testidPrefix="form"}){
  ...
  return <form onSubmit={submit} data-testid={`${testidPrefix}-form`} className="space-y-10">
    ...
    <div><button type="submit" data-testid={`${testidPrefix}-submit`} disabled={loading} className="group flex w-full sm:w-auto items-center justify-center gap-2 bg-[#70CCFF] text-[#25292C] px-9 py-4 rounded-full font-extrabold tracking-wide hover:bg-[#A8E1FF] transition-colors duration-300 disabled:opacity-60">{loading?<><Loader2 size={18} className="animate-spin"/>Sending…</>:<>{submitLabel}<ArrowRight size={18} className="group-hover:translate-x-1 transition-transform"/></>}</button><p className="mt-4 text-xs text-[#59636A]">Information submitted here is handled according to our <Link to="/privacy" className="font-bold text-[#1683BD] hover:underline">Privacy Policy</Link>.</p></div>
  </form>
}
